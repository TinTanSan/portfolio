...
export default function Experience({experience}: props) {
  return (
    <div className="flex flex-col border-2 px-2 items-center">
        ...
        <div className='flex w-3/4 md:w-[98%] h-0.5 bg-black' />
        <div className="text-xs md:text-sm">{experience.duration} | {experience.location}</div>
        ...
        <div className='flex flex-col w-full h-full'>
            ...
            {experience.contributions.map((contribution, i)=>{
                return (
                    <ul key={i} className='flex flex-col list-inside'>
                        <h3 className='flex font-bold pl-2 text-[0.8rem] md:text-lg'>{contribution.title}</h3>
                        {contribution.details.map((detail, j)=>
                            <div className='flex gap-1 pl-5 text-[0.75rem] md:text-[0.95rem]'>
                                -
